function [crcimg,limitval]=CreateCambellRobsonChart(img_size,sfs,contrasts,center,amplitude,pix_per_deg,limitBpc)

% fucntion [crcimg,limitval]=CreateCambellRobsonChart(img_size,:sfs,:contrasts,:center,:amplitude,:pix_per_deg,:limitBpc)
% (: is optional)
%
% This function generates a Combell-Robson Chart, aka Contrast-Sensitivity function.
...
if numel(sfs)~=2, error('sfs should be [min,max]. check the input variable.'); end
...
% drawing the output Cambell-Robson Chart on the image canvas
...
%{
% just for reference, BitsPlusCSFDemo's case:

function [crcimg,limitval]=CreateCambellRobsonChart(img_size,sfs,contrasts,center,amplitude,limitBpc)

% fucntion [crcimg,limitval]=CreateCambellRobsonChart(img_size,:sfs,:contrasts,:center,:amplitude,:limitBpc)
% (: is optional)
%
% This function generates a Combell-Robson Chart, aka Contrast-Sensitivity function.
% The output "crcimg" contains the image of the standard CSF.
...
%
% [input]
% img_size  : the output image size, [row,col] in pixels.
% sfs       : (optional) spatial frequency (cycles per img_size(2)) range, [min,max]. [1,77] by default.
% contrasts : (optional) the ouput image contrast, [min,max]. [1/512,1]by default.
% center    : (optional) image maen value, grayscale, [val]. [127/255] by default.
% amplitude : (optional) image amplitude, [val]. [127/255] by default.
...
%
% [example]
% >> img=CreateCambellRobsonChart([1080,1920],[1,77],[1/1024,1],127/255,127/255,8);
%
% [reference]
% BitsPlusCSFDemo.m in Psychtoolbox (PTB3).
%
...
if nargin<4 || isempty(center), center=127/255; end
if nargin<5 || isempty(amplitude), amplitude=127/255; end
if nargin<6 || isempty(limitBpc), limitBpc=8; end
...
CT=contrasts(1)/contrast_bump;
crcimg=zeros(img_size);

% drawing the Cambell-Robson Chart with sweeping the image canvas
for ii=0:1:img_size(1)-1
  CT=CT*contrast_bump;  % increase contrast by a constant factor
  SF=(sfs(1)/img_size(2))/sfrequency_bump; % intialize to base lowest SF
