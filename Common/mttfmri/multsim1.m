...
%      
%   020913  TTL  changing handling of rank deficiency to just set eff = 0
	...
	if 1
	  ...
	  for pnum = 1:numperm_all
	    ...
	    for iorder = 1:norders
	      ...
	      %CHECK THE RANK
	      rX = rank(squeeze(PS(iorder,:,:))*X);
	      if(rX < size(X,2));
		%                        if(0)
		fprintf('rank deficient; itype %d blocknum %d perm %d iorder %d\n',...
			itype, blocknum,pnum,iorder);
		rd = rd + 1;
		effmat(itype,iorder,1,pnum,blocknum) = 0;
	      else
			  
				   
		%Full COVARIANCE MATRICES
		K = X'*squeeze(PS(iorder,:,:))*X;
		CX = inv(K);
		CZ = inv(hmat'*K*hmat);
		...
		for k = 1:nevents
		  ...
		  if (dorand==1)
		    reffmat(itype,iorder,k+1,pnum,1) = trace(A*CX*A');
		    rdetmat(itype,iorder,k+1,pnum,1) = D*CZ*D'* ...
		      hnorm2;
		  elseif (domseq == 1)
		    meffmat(itype,iorder,k+1,pnum,1) = trace(A*CX*A');
		    mdetmat(itype,iorder,k+1,pnum,1) = D*CZ*D'* ...
		      hnorm2;		    
		  else
		    effmat(itype,iorder,k+1,pnum,blocknum) = trace(A*CX*A');
		    detmat(itype,iorder,k+1,pnum,blocknum) = D*CZ*D'* ...
		      hnorm2;
		  end
		end
		...
		for k = 1:ncontrasts
		  ...
		  A = kron(D,eye(nummods));
		  if (dorand == 1)
		  reffmat(itype,iorder,k+nevents+1,pnum,1) = trace(A*CX*A');
		  rdetmat(itype,iorder,k+nevents+1,pnum,1) = ...
		      D*CZ*D'*hnorm2;
		  elseif (domseq == 1)
		    meffmat(itype,iorder,k+nevents+1,pnum,1) = trace(A*CX*A');
		    mdetmat(itype,iorder,k+nevents+1,pnum,1) = ...
		      D*CZ*D'*hnorm2;
		  else
		    effmat(itype,iorder,k+nevents+1,pnum,blocknum) = trace(A*CX*A');
		    detmat(itype,iorder,k+nevents+1,pnum,blocknum) = ...
		      D*CZ*D'*hnorm2;
		  end
		end
		...
		% overall efficiency
		if (dorand == 1)
		  reffmat(itype,iorder,1,pnum,1) = ...
		        1/mean(reffmat(itype,iorder,2:neffdet,pnum,1));
		  rdetmat(itype,iorder,1,pnum,1) = ...
		        1/mean(rdetmat(itype,iorder,2:neffdet,pnum, ...
				      1));
		  
		elseif (domseq == 1)
		  meffmat(itype,iorder,1,pnum,1) = ...
		        1/mean(meffmat(itype,iorder,2:neffdet,pnum,1));
		  mdetmat(itype,iorder,1,pnum,1) = ...
		        1/mean(mdetmat(itype,iorder,2:neffdet,pnum, ...
				      1));
		  
		else
		  effmat(itype,iorder,1,pnum,blocknum) = ...
		        1/mean(effmat(itype,iorder,2:neffdet,pnum,blocknum));
		  detmat(itype,iorder,1,pnum,blocknum) = ...
		        1/mean(detmat(itype,iorder,2:neffdet,pnum, ...
				      blocknum));
		end
	      end % if rank deficient
...
if 1
filename = sprintf('ne%dl%sno%dz%dds%dc%d',nevents, ...
		   no_blanks(num2str(lorders)),numones,zeroarea,doshift,doclump);
